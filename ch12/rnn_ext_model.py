...
from ch11.rnn_lstm_model import RnnLstmModel
...
class RnnExtModel(RnnLstmModel):
    ...
    def backprop_seqwrap_layer(self, G_y, hconfig, pm, aux):
        ...
        for n in reversed(range(hconfig[2:])):
    ...
    def init_parameters(self, hconfigs):
        ...
        if len(hconfigs) > 0 and self.get_layer_type(hconfigs[-1]) in ['rnn', 'lstm']:
            self.seqout = True
    ...
    def backprop_neuralnet(self, G_output, aux):
        ...
        for n in reversed(range(self.hconfigs)):
